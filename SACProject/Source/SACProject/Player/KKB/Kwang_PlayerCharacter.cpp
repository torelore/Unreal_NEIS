...
float AKwang_PlayerCharacter::TakeDamage(float DamageAmount, FDamageEvent const& DamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
	...
	AGrux_StampedeAttackCol* GruxCol = Cast<AGrux_StampedeAttackCol>(DamageCauser);

	if (IsValid(GruxCol)) {
		FVector CauserZero = DamageCauser->GetActorLocation();
		CauserZero.Z = GetActorLocation().Z;

		FVector launchDir = (CauserZero - GetActorLocation()).GetSafeNormal();

		LaunchCharacter(-launchDir * m_DashDistance, false, false);
	}
